...
def _run(
    src: Path, snk: Path, threshold_db: float, agc: Step | None = None
) -> np.ndarray:
    path: list[Step] = [] if agc is None else [agc]
    path.append(SquelchStep(threshold_db=threshold_db, alpha_symbols=1.0))
    pipe = compile_modem(
...
def test_squelch_never_gates_the_stream() -> None:
    ...
    with pytest.raises(ValidationError):
        stage_registry()["squelch"].step_model.model_validate(
            {"threshold_db": -20.0, "gate": True}
        )


def test_squelch_rejects_invalid_time_constants() -> None:
    model = stage_registry()["squelch"].step_model
...
def test_a_short_agc_window_defeats_the_squelch(tmp_path: Path) -> None:
    ...
    n = 8192
    src = _bursty(tmp_path, n)
    short = AgcStep(mode=AgcMode.POWER, window_symbols=16.0)
    long = AgcStep(mode=AgcMode.POWER, window_symbols=4096.0)

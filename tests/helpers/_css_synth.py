"""Shared CSS explicit-header fixtures: the off-air SF11 golden symbols and
encoders that run the decoder's coding primitives in reverse."""
...
def encode_header(payload_len: int, cr: int, has_crc: int) -> list[int]:
    ...
    sf_app = sf - sf_red
    header_cr = HEADER["header_cr"]
    cw_len = header_cr + HEADER["data_bits"]
    data_int = (payload_len << 4) | (cr << 1) | has_crc
    masks = HEADER["header_parity"]
    parity = 0
    ...
    for i, src in enumerate(perm):
        chunk[src] = deint[i]
    syms = []
    for s in range(HEADER["header_symbols"]):
        gv = 0
...
def encode_payload(nibbles: Sequence[int], cr: int, sf_app: int) -> list[int]:
    ...
    cw_len = cr + data_bits
    fec = coding.parity_for_cr(PARITY_MASKS, cr)
    perm = coding.diag_deinterleave_perm(sf_app, cw_len)
    syms = []
    for b in range(len(nibbles) // sf_app):
        ...
        for s in range(cw_len):
            ...
            if sf_app == sf:
                syms.append(
                    (coding.gray_decode(gv) + HEADER["full_offset"]) % (1 << sf)
                )
            else:

...
What this path CAN deliver is the positive, which noise cannot fake: noise and
near-silence through this same modem shape scored margin 1.15 with no evidence
at all, and a 3 dB 4-level signal 1.32. So every window must carry a soft_eye
POSITIVE at or above the quality module's 4.0 multilevel bar, and a demod or
threshold change narrowing any single burst fails here. The same reading on
synthetic captures — a 4-level eye scoring separation 18.0 against noise's
forced-fit 3.20 — is gated without the asset by
tests/integration/engine/quality/test_quality_wiring.py.

READ THIS BEFORE BLAMING THE DEMOD if a burst goes red. The separations behind
the bar, ~5.07-8.12 at fit order 4, were recorded when these windows were
chosen and have NOT been re-measured since: this checkout does not hold the
capture (it is regenerated by hand — see make_dmr_slice.py — and is in no
asset manifest). One input to the positive has moved under that fixture:
whiteness is now centred, (raw - m^2)/(1 - m^2), where it was the bare lagged
sign mean, and _positive_value gates on it. A sign-biased burst can therefore
read a higher correlation than it did when 5.07-8.12 were taken (raw ~0 at 30%
bias moves 0.0 -> 0.099; ~37% bias would cross the 0.15 whiteness bar and
withhold the positive with the eye itself unchanged). Check lane.whitened
before the DSP, and record the per-burst separations you measure here."""

...
download it and place it at SRC, then run:
    uv run python tests/bits/make_drm_slice.py

Conversion: mix the 12 kHz IF to DC, low-pass (127-tap Hamming-windowed sinc,
6 kHz cutoff), decimate x4 -> 12 kHz complex64 — the baseband tests/bits/_drm.py's
...
_DIR = Path(__file__).resolve().parents[1] / "artifacts" / "assets" / "DRM"

...
def test_dedupe_glob_metacharacters(tmp_path: Path) -> None:
    ...
    assert first == tmp_path / "captures" / "ISM[2.4GHz]"
    ...
    assert second == tmp_path / "captures" / "ISM[2.4GHz]-1"
...
def test_new_scene_path(tmp_path: Path) -> None:
    ...
    assert p == tmp_path / "scenes" / "downtown.yaml"
...
def test_new_pipeline_path(tmp_path: Path) -> None:
    ...
    assert p == tmp_path / "pipelines" / "demod.yaml"
    assert p.parent.is_dir()

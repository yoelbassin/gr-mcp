...
from marconi.mcp.tools import run_rx_tool, stream_stats
...
def _bits(seed: int) -> np.ndarray:
    return np.random.default_rng(seed).integers(0, 2, _N_BITS).astype(np.uint8)


def _qpsk(seed: int) -> np.ndarray:
    """Genuine QPSK: Gray-mapped points, RRC pulse-shaped — one ring."""
    return synth.shaped_constellation(_bits(seed), scheme="psk", order=4, sps=_SPS)


def _fm(deviation: float, seed: int) -> np.ndarray:
    """The false-lock fixture: continuous-phase FM forced through a PSK demod."""
    return synth.cpfsk(_bits(seed), sps=_SPS, deviation=deviation, sample_rate=_FS)


def _constant_modulus_ratio(signal: np.ndarray, tmp_path: Path, name: str) -> float:
    iq = synth.write(tmp_path / name, signal)
    res = run_rx_tool(_QPSK_RX, _FS, capture_path=str(iq), timeout=60.0)
...
def test_real_qpsk_is_tight_but_fm_through_psk_is_high_cmr(
    ...
    cmr_ok = [
        _constant_modulus_ratio(_qpsk(seed), tmp_path, f"qpsk_{seed}.cf32")
    ...
    cmr_bad = [
        _constant_modulus_ratio(_fm(dev, seed), tmp_path, f"fm_{dev}_{seed}.cf32")
